...
<div id="canvas-holder">
	...
	<div id="diagramright">
		...
		<canvas id="coursecompletion"></canvas>
	</div>
	...
	{if isset($userinfo_row)}
		{foreach from=$userinfo_row item=course key=user_id}
				...
				<tr>
					...
					<td style="border-width:1px;">
						{if isset($row.enrolleddate) and $row.enrolleddate}
							{$row.enrolleddate|date_format:"%d/%m/%Y"}
						{/if}
					</td>
					<td style="border-width:1px;">
						{if $row.percentage=='100%' and isset($row.completiondate) and $row.completiondate}
							{$row.completiondate|date_format:"%d/%m/%Y"}
						{/if}
					</td>

					{* FIX: Check if profile_result exists before trying to loop through it. *}
					{if isset($row.profile_result) and is_array($row.profile_result)}
...
var bardata = {
    labels: course_completion_diagram_value_coursename.split(",").filter(Boolean), // Added filter to remove empty labels
    datasets : [
        {
            label: 'Course Completed',
            backgroundColor: bar_color_completed,
            data : course_completion_diagram_value_string.split(",").filter(Boolean)
        }
    ]
};
