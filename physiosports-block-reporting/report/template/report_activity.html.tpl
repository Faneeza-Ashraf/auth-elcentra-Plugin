...
{/php}
<div style='text-align:right;margin-bottom:10px'><a href="#" class="export btn btn-primary">Export Excel/CSV</a> </div>
	...
	{if isset($userinfo_row)}
		{foreach from=$userinfo_row item=row key=result}
				<tr>
					...
					<td style="border-width:1px;">{$row.enrolleddate|date_format:"%d/%m/%Y"}</td>
					{if isset($row.module) and is_array($row.module)}
...
{literal}
 <script>
	...
	$.tablesorter.addParser({
        ...
        format: function(s) {
            var date = s.split('/');
            // Check for valid date parts to avoid errors
            if (date.length === 3) {
