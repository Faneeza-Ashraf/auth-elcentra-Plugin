...
class generalized_filter_date extends generalized_filter_type {
    ...
    public function __construct($uniqueid, $alias, $name, $label, $advanced, $field, $options = array()) {
        parent::__construct($uniqueid, $alias, $name, $label, $advanced,
                !empty($options['help']) ? $options['help'] : array('date', $label, 'local_eliscore'));
...
protected function reinit_js() {
    $jscode = 'YUI().use("node", "event", function(Y) {
            // Re-initialize Moodle calendar objects.
            M.form_moodlecalendar = null;
            M.form.dateselector = null;
            Y.all("#dateselector-calendar-panel").remove();

            // The unique ID for this specific filter instance\'s wrapper div.
            var wrapperId = "#filter-date-wrapper-'.$this->_uniqueid.'";

            // CORRECTED: Use Y.all() to find EVERY calendar button within this wrapper.
            var calendarButtons = Y.all(wrapperId + " .fdate_selector a");

            if (calendarButtons.size() > 0) {
                // CORRECTED: Loop through each button found and apply the fix.
                calendarButtons.each(function(buttonNode) {
                    buttonNode.on("click", function(e) {
                        // Stop the click event to prevent the report from loading.
                        e.preventDefault();
                        e.stopPropagation();
                    });
                });
    ...
    function setupForm(&$mform) {
      ...
      // This is in the setupForm() function
    $mform->addElement('html', '<div id="filter-date-wrapper-'.$this->_uniqueid.'">');
    ...
    function check_data($formdata) {
        ...
        if (!property_exists($formdata, $sck) and !property_exists($formdata, $eck)) {
            return false;
        }
        ...
        $data = array();
        if (property_exists($formdata, $sck) && !empty($formdata->$sck)) {
            $data['after'] = $formdata->$sdt;
        } else {
            $data['after'] = 0;
        }
        if (property_exists($formdata, $eck) && !empty($formdata->$eck)) {
            $data['before'] = $formdata->$edt;
        } else {
            $data['before'] = 0;
        }
        if (property_exists($formdata, $never) && !empty($formdata->$never)) {
