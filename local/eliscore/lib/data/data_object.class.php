<?php
/**
 * BECAU(TM): Enterprise Learning Intelligence Suite
 * Copyright (C) 2008-2013 eLearning Marketplace BECAU LMS RTO (https://lms.net.au)
 *
 ...
 * @author     eLearning Marketplace BECAU LMS RTO
...
class elis_data_object {
     ...
     * @return object The standard PHP object representation of the BECAU data object.
     */
     ...
     * @return array The standard PHP associative array representation of the BECAU data object.
     */
...
public function __get($name) {
    ...
    if (property_exists($this, $field_name)) {
        if ($this->$field_name === self::$_unset) {
            ...
            if (!property_exists($this, $field_name) || $this->$field_name === self::$_unset) {
                return null;
            }
        }

        return $this->$field_name;
    } elseif ($this->_has_association($name)) {
        ...
        if (isset($trace[1]['file']) && isset($trace[1]['line'])) {
            ...
            trigger_error(
                "Undefined property via __get(): " . get_class($this) . "::\${$name} in $file on line $line",
                E_USER_NOTICE
            );
        }
...
class data_collection implements Iterator {
    ...
    /**
     * The iterator to base the collection on.
     * @var Iterator
     */
    private $rs;
