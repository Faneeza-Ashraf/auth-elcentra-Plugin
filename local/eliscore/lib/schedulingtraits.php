    ...
    public function save_elis_scheduled_task($taskname, $component, $callfile, $callfunction, $data) {
        ...
        if (empty($data['period'])) {
            ...
            if ($recurrencetype == static::$RECURRENCE_SIMPLE) {
                ...
                $minute = (int)date('i', $startdate);
                $hour   = (int)date('H', $startdate);  //Updated by Faneeza Muskan
                ...
                switch ($data['schedule']['frequencytype']) {
                    ...
                    case self::$FREQ_MONTH:
                        ...
                        $day = (int)date('d', $startdate);  //Updated by Faneeza Muskan
                        break;
...
trait elisschedulingpagetrait {
    ...
    public function display_step_schedule($errors) {
        ...
        if (!empty($workflowdata['period'])) {
            ...
            if (!isset($workflowdata['recurrencetype']) || $workflowdata['recurrencetype'] != self::$RECURRENCE_PERIOD) {  //Updated by Faneeza Muskan
        ...
        if (isset($data->recurrencetype)) {
            ...
            if ($data->recurrencetype == self::$RECURRENCE_SIMPLE) {
                ...
                if (isset($workflowdata['schedule']['enddate'])) {
                    ...
                    $data->runtype = (!empty($workflowdata['schedule']['runsremaining']) || $data->frequency != 1 || $data->frequencytype != self::$FREQ_DAY) ? 2 : 0; //Updated by Faneeza Muskan
                }
            //} else if ($data->recurrencetype == elisschedulingworkflowtrait::$RECURRENCE_CALENDAR) {
            } else if ($data->recurrencetype == self::$RECURRENCE_CALENDAR) { //Updated by Faneeza Muskan
    ...
    public function get_submitted_values_for_step_schedule() {
        ...
        if (!empty($data->period)) {
            ...
            //if ($data->recurrencetype == elisschedulingworkflowtrait::$RECURRENCE_SIMPLE) {
            if ($data->recurrencetype == self::$RECURRENCE_SIMPLE) { //Updated by Faneeza Muskan
