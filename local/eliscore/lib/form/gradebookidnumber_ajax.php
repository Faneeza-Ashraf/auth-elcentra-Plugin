...
if ($mode != 'gradebook') {
    ...
    if ($items->valid()) {
        ...
        foreach ($items as $item) {
            $item->icon = $OUTPUT->icon_url('icon', 'mod_'.$item->itemmodule);
            print_item($item);
        }
    ...
    if ($items->valid()) {
        ...
        foreach ($items as $item) {
            switch ($item->itemtype) {
                case 'category':
                    $item->icon = $OUTPUT->icon_url('f/folder');
                    break;
                case 'manual':
                    $item->icon = $OUTPUT->icon_url('t/manual_item');
                    break;
                default:
                    $item->icon = $OUTPUT->icon_url('f/unknown');
            }
            print_item($item);
        }
        echo html_writer::end_tag('ul');
