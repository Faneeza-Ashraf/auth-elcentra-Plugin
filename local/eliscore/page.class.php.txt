    ...
    public function required_param($name, $type=PARAM_CLEAN) {
        if ($this->params !== null) {
            if (isset($this->params[$name])) {
                ...
                echo $OUTPUT->fatal_error(get_string('missingparam', 'error', $name));  //Updated by Faneeza Muskan
            }
        } else {
    ...
    public function run() {
        ...
        if ($this->can_do($action)) {
            ...
            if (method_exists($this, "do_{$action}")) {
                ...
                echo $OUTPUT->fatal_error(get_string('unknown_action', 'local_eliscore', $action));  //Updated by Faneeza Muskan
            }
        } else {
            print_error('nopermissions', '', '', $action);
            echo $OUTPUT->fatal_error(get_string('nopermissions', 'moodle', $action));  //Updated by Faneeza Muskan
        }
    }
