...
abstract class workflowpage extends elis_page {
    ...
    function get_next_step() {
        ...
        $current_index = array_search($curr_step, $keys);

        if ($current_index !== false) {
            // Check if there is a next key in the array.
            if (isset($keys[$current_index + 1])) {
                // Return the next step's key.
                return $keys[$current_index + 1];
            } else {
                // This was the last step, so the next action is to finish.
                return workflow::STEP_FINISH;
            }
        }
        print_error('workflow_invalidstep', 'local_eliscore');  //Updated by Faneeza Muskan
