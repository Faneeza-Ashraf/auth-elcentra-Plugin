 ...
 * Copyright (C) 2016-2023 eLearning Marketplace BECAU LMS RTO (https://lms.net.au)
 *
...
// BECAU-9478: process 1,000 records at a time (down from 10K).
...
function user_activity_etl_cron($taskname = '', $duration = 0, &$etlobj = null) {
    ...
    try {
        // Run the ETL cron process
        $etlobj->cron();
    } catch (Exception $e) {
        // Log or handle any exceptions that occur during the cron process
        error_log('Error in user_activity_etl_cron: ' . $e->getMessage());
    }
}
