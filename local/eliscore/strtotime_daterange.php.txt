...
class generalized_filter_strtotime_daterange extends generalized_filter_type {
    ...
    var $intervals = array(); // Added by Faneeza Muskan
    ...
    public function __construct($uniqueid, $alias, $name, $label, $advanced, $field, $options = array()) {
        ...
        $this->fromlabel = isset($options['fromlabel']) ? $options['fromlabel'] : get_string('fromdate', 'core'); // Changed by Faneeza Muskan
        //$this->tolabel = isset($options['tolabel']) ? $options['tolabel'] : get_string('to', 'moodle');
        $this->tolabel = isset($options['tolabel']) ? $options['tolabel'] : get_string('todate', 'core'); // Changed by Faneeza Muskan
